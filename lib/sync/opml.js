  ...
  try {
    ...
    for (const blog of blogs) {
      const result = await upsertBlog(application, {
        ...blog,
        // A flat OPML (feeds listed directly, with no folder outlines) carries no
        // category, and the frontend builds its tabs from categories — so those
        // blogs would import and then be unreachable. Fall back to the source's
        // own name, which reads better as a tab than a generic bucket.
        category: blog.category || source.name || "",
        sourceId: source._id,
      });
