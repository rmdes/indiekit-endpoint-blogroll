 ...
 */
/**
 * Conventional feed endpoints. When a feed lives at one of these, its origin is
 * a safe guess for the site that publishes it.
 */
const CONVENTIONAL_FEED_PATH =
  /^\/(?:(?:index|feed|rss|atom|feeds?)(?:\.(?:xml|json|rss|atom))?|feeds?\/[\w.-]+)\/?$/i;

/**
 * Derive the site a feed belongs to, or nothing if it can't be known.
 *
 * Previously this returned the feed URL's origin unconditionally, which invents
 * a homepage for feeds that have none — a webhook at /webhook/foo became a link
 * to the automation tool's front page, presented to visitors as if it were a
 * blog. Returning "" instead lets the frontend fall back to the feed URL, which
 * is at least true.
 * @param {string} feedUrl - Feed URL
 * @returns {string} Site URL, or "" when it cannot be inferred
 */
export function extractSiteUrl(feedUrl) {
  try {
    const parsed = new URL(feedUrl);
    const path = parsed.pathname;

    // Subscribed directly to a site (h-feed discovery), or a well-known feed path.
    if (path === "" || path === "/" || CONVENTIONAL_FEED_PATH.test(path)) {
