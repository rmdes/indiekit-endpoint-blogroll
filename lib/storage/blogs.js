...
/**
 * Bucket shown for blogs with no category. The frontend builds its tabs from
 * categories, so a blog without one used to be silently unreachable — ingested,
 * stored, and invisible. Grouping them under a named bucket instead means every
 * blog is always browsable, whatever gaps a future sync path leaves.
 * Matches the spelling already used by generateOpml().
 */
export const UNCATEGORIZED = "Uncategorized";

/**
 * Blog fields a user may edit by hand. Once edited, sync must not overwrite
 * them — see manualFields handling in upsertBlog().
 */
export const LOCKABLE_FIELDS = ["title", "category", "siteUrl"];
...
export async function getBlogs(application, options = {}) {
  ...
  if (category) {
    // getCategories() presents empty/null/missing categories under one bucket,
    // so selecting that bucket has to match all three — not a literal field value.
    query.category =
...
export async function getCategories(application) {
  ...
  return collection
    .aggregate([
      { $match: { hidden: { $ne: true }, status: { $ne: "deleted" } } },
      {
        // Empty, null and missing categories all collapse into one visible
        // bucket rather than being filtered out of the tab list entirely.
        $group: {
          _id: {
            $let: {
              vars: { c: { $ifNull: ["$category", ""] } },
              in: { $cond: [{ $eq: ["$$c", ""] }, UNCATEGORIZED, "$$c"] },
            },
          },
          count: { $sum: 1 },
        },
...
export async function upsertBlog(application, data) {
  ...
  // Fields the user has edited by hand are theirs to keep: drop them from $set
  // so a resync cannot silently revert the edit. Clearing the field in the UI
  // removes it from manualFields, handing control back to the source.
  const existing = await collection.findOne(filter, {
    projection: { manualFields: 1 },
  });
  for (const field of existing?.manualFields || []) {
